...
const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'First Development Destek Sistemi', iconURL: 'https://i.imgur.com/ayGmpzF.png' })
    .setTimestamp()
    ...
    .addFields(
        ...
        const roleMenu = new ActionRowBuilder().addComponents(
            new RoleSelectMenuBuilder()
                ...
                .setPlaceholder(BasvuruUserData.YetkiliRole ? message.guild.roles.cache.get(BasvuruUserData.YetkiliRole)?.name || 'Yetkili Rolü ayarlanmamış' : 'Yetkili Rolü ayarlanmamış')
        );
        ...
        const DestekChannelMenu = new ActionRowBuilder().addComponents(
            new ChannelSelectMenuBuilder()
                ...
                .setChannelTypes([0])
                .setPlaceholder(BasvuruUserData.DestekChannel ? message.guild.channels.cache.get(BasvuruUserData.DestekChannel)?.name || 'Destek kanalı ayarlanmamış' : 'Destek kanalı ayarlanmamış')
        );
        ...
        const logChannelMenu = new ActionRowBuilder().addComponents(
            new ChannelSelectMenuBuilder()
                ...
                .setChannelTypes([0])
                .setPlaceholder(BasvuruUserData.logChannel ? message.guild.channels.cache.get(BasvuruUserData.logChannel)?.name || 'Log kanalı ayarlanmamış' : 'Log kanalı ayarlanmamış') 
        );
        ...
        const DestekKategoriMenu = new ActionRowBuilder().addComponents(
            new ChannelSelectMenuBuilder()
                ...
                .setChannelTypes([4])
                .setPlaceholder(BasvuruUserData.DestekKategori ? message.guild.channels.cache.get(BasvuruUserData.DestekKategori)?.name || 'Kategori ayarlanmamış' : 'Kategori ayarlanmamış') 
        );

        const buttons = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId('select_send_message')
